...
class TalentFilter(FilterSet):
    roles = CharFilter(method='filter_roles')  # Custom filter for roles array

    class Meta:
        model = Talent
        fields = ['roles', 'eye_color', 'hair']

    def filter_roles(self, queryset, name, value):
        """
        Custom filtering for the 'roles' list field to support multiple role values.
        """
        roles = value.split(',')
        for role in roles:
